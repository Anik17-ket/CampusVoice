...
1. **Password Storage:** Passwords are now hashed using bcrypt (configurable `SALT_ROUNDS` via .env). **Existing user records created before this change will no longer authenticate**; clear or migrate the user collection when upgrading.
...
- Passwords are hashed on the backend using `bcrypt`. Update `.env` with `SALT_ROUNDS` if custom salt rounds are desired.
